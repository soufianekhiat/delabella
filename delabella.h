...
struct SDBVertex
{
	SDBVertex* next; // next in internal / boundary set of vertices
	SDBSimplex* sew; // one of triangles sharing this vertex
	Scalar x, y; // coordinates (input copy)
	Integer i; // index of original point

	inline const SDBSimplex* StartIterator( SDBIterator* it/*not_null*/ ) const;
};

struct SDBSimplex
{
	SDBVertex* v[ 3 ];  // 3 vertices spanning this triangle
	SDBSimplex* f[ 3 ]; // 3 adjacent faces, f[i] is at the edge opposite to vertex v[i]
	SDBSimplex* next; // next triangle (of delaunay set or hull set)
	...
	inline const SDBSimplex* StartIterator( SDBIterator* it/*not_null*/, int around/*0,1,2*/ ) const;
};

struct SDBIterator
{
	const SDBSimplex* current;
	int around;

	const SDBSimplex* Next()
	{
		int pivot = around + 1;
		if ( pivot == 3 )
			pivot = 0;

		SDBSimplex* next = current->f[ pivot ];
		SDBVertex* v = current->v[ around ];
	...
	const SDBSimplex* Prev()
	{
		int pivot = around - 1;
		if ( pivot == -1 )
			pivot = 2;

		SDBSimplex* prev = current->f[ pivot ];
		SDBVertex* v = current->v[ around ];
...
struct SDB
{
	...
	const SDBSimplex* GetFirstDelaunaySimplex() const; // valid only if Triangulate() > 0
	const SDBSimplex* GetFirstHullSimplex() const; // valid only if Triangulate() > 0
	const SDBVertex*  GetFirstBoundaryVertex() const; // if Triangulate() < 0 it is list, otherwise closed contour! 
	const SDBVertex*  GetFirstInternalVertex() const; // valid only if Triangulate() > 0

	// given input point index, returns corresponding vertex pointer 
	const SDBVertex*  GetVertexByIndex(Integer i) const;
	...
	Integer FloodFill(bool invert, const SDBSimplex** exterior = 0, int depth = 0);
	...
	Integer Polygonize(const SDBSimplex* poly[/*GetNumOutputIndices()/3*/] = 0); 
...
inline const SDBSimplex* SDBSimplex::StartIterator( SDBIterator* it/*not_null*/, int around/*0,1,2*/) const
{
	it->current = this;
	it->around = around;
	return this;
...
inline const SDBSimplex* SDBVertex::StartIterator( SDBIterator* it/*not_null*/) const
{
	it->current = sew;
	if (sew->v[0] == this)
		it->around = 0;
